    ...
    setData = async()=> {
        if(!await (this.state.month===""&&this.state.year==="")){
            if(!(this.state.selectEmpID==="")){
                ...
                axios.get("http://localhost:8400/department/show/"+this.state.selectEmpID).then(res => {
                if (res.data.success) {
                            ...
                            height: "50%"
                                ...
                                >
                                    ...
                                    {this.state.empList.map((emp, index) => (
                                        <option value={emp.empid} data-order={emp.empName}>{emp.empName}</option>
                                    ))}
